  ...
  return (
    <div className={`${step === 4 ? "max-h-[300px]" : "max-h-[160px]"} overflow-y-auto custom-scroll mt-2`}>
      {selectedFiles && selectedFiles?.length === 0 ? (
        ...
        <div key={index} className="flex justify-between items-center text-center space-x-2 py-2">
          <div className='flex items-center space-x-1'>
            <div className='mr-1'>{fileIcons[getMime(file.type)]}</div>
            <div>
              <h6 className="heading !text-left !font-[700]">{file.name ? file.name : file?.folderName}</h6>
